    ...
    for page in ec2.get_paginator("describe_volumes").paginate():
        for volume in page.get("Volumes", []):
            resources.append({
                "id": volume["VolumeId"],
                "type": "EBS",
                "status": "attached" if volume.get("Attachments") else "unused",
                "source": "aws",
                "details": {
...
def list_resources(db: Session):
    return _aws_resources() if settings.data_source == "aws" else _demo_resources(db)
...
def summarize_resources(resources):
    return {
        ...
        "underutilized": sum(1 for item in resources if item["status"].lower() == "underutilized"),
        "by_type": {
