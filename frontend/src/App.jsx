  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [dashboardData, costsData, resourceData] = await Promise.all([
        dashboardResponse.json(), costsResponse.json(), resourceResponse.json(),
      ]);
    ...
    <div className="app-shell">
      <main className="container">
        <header className="topbar">
          ...
          <div className="environment"><span className="status-dot" /><span>{dashboard?.data_source === "aws" ? "AWS" : "Demo"}</span><span className="separator">•</span><span>us-east-1</span></div>
        </header>

        <section className="hero">
          <div><p className="eyebrow">AWS COST INTELLIGENCE</p><h2>See where your AWS spend goes — and where you can save.</h2><p className="hero-description">CloudCostOps combines AWS cost data, resource inventory, and optimization rules into one operational view.</p></div>
        ...
        <section className="kpi-grid">
          <div className="kpi-card primary"><div className="kpi-header"><span>{days}-Day AWS Spend</span><span className="kpi-icon">$</span></div><strong>{formatCurrency(costs?.total, currency)}</strong><p>{dashboard?.data_source === "aws" ? "AWS Cost Explorer" : "Demo data"}</p></div>
          <div className="kpi-card"><div className="kpi-header"><span>Previous Baseline</span><span className="kpi-icon">↗</span></div><strong>{dashboard?.previous_month_cost == null ? "—" : formatCurrency(dashboard.previous_month_cost, currency)}</strong><p>Historical comparison</p></div>
          <div className="kpi-card savings"><div className="kpi-header"><span>Potential Savings</span><span className="kpi-icon">✦</span></div><strong>{formatCurrency(dashboard?.potential_savings, currency)}</strong><p>Validated recommendations only</p></div>
          <div className="kpi-card"><div className="kpi-header"><span>Resources</span><span className="kpi-icon">◌</span></div><strong>{dashboard?.resources?.total || 0}</strong><p>{dashboard?.resources?.unused || 0} unused · {dashboard?.resources?.underutilized || 0} underutilized</p></div>
        </section>
        ...
        <section className="panel"><div className="panel-header"><div><p className="panel-eyebrow">AWS RESOURCES</p><h3>Resource Inventory</h3></div><span className="period-badge">{resources.length} resources</span></div><div className="table-wrapper"><table><thead><tr><th>Resource</th><th>Type</th><th>Status</th><th>Source</th></tr></thead><tbody>{resources.length ? resources.map((resource) => <tr key={`${resource.type}-${resource.id}`}><td><strong>{resource.id}</strong></td><td>{resource.type}</td><td><span className={`resource-status ${String(resource.status).toLowerCase()}`}>{resource.status}</span></td><td>{resource.source}</td></tr>) : <tr><td colSpan="4"><div className="empty-state">No AWS resources found.</div></td></tr>}</tbody></table></div></section>

        <section className="content-grid resource-grid"><div className="panel"><div className="panel-header"><div><p className="panel-eyebrow">RESOURCE HEALTH</p><h3>Inventory Summary</h3></div></div><div className="resource-stats"><div className="resource-stat"><span>Total</span><strong>{dashboard?.resources?.total || 0}</strong></div><div className="resource-stat warning"><span>Unused</span><strong>{dashboard?.resources?.unused || 0}</strong></div><div className="resource-stat"><span>Underutilized</span><strong>{dashboard?.resources?.underutilized || 0}</strong></div></div></div><div className="panel savings-panel"><p className="panel-eyebrow">OPTIMIZATION</p><div className="savings-number">{formatCurrency(dashboard?.potential_savings, currency)}</div><p>Current recommendation savings. Values remain zero until defensible AWS pricing or usage data is available.</p></div></section>
        ...
        <footer><span>CloudCostOps</span><span>•</span><span>AWS Cost Intelligence Platform</span><span>•</span><span>{dashboard?.data_source === "aws" ? "AWS live integration" : "Local demo mode"}</span></footer>
      </main>
    </div>
  );
}
